...
# set factor
no_metrics_split = split(sister_no_metrics, f=sister_no_metrics$state)
# loop over traits
all_pvalues = c()
for(i in 1:(length(no_metrics_split)-1) ){
  for(j in (i+1):length(no_metrics_split) ){
    # select data by state
    i_df = no_metrics_split[[i]]
    j_df = no_metrics_split[[j]]
    # state name
    ...
    # set variable 
    response = ij_df$intercept_no
    factor = ij_df$state
    # test
    pvalue = permutation_test(factor = factor, response = response, stats="mean", iter=999, out_dir="2_sister_hypervolume",  name= paste(pair_name, ".tiff", sep=""))
